...
```{r}
CC_Data <- read_csv("/home/dcclxxvii/Data/creditcard.csv")
CC_Data <- CC_Data[1:1000,]

head(CC_Data, n=3)

```
```{r}
CC_Data_scale <- scale(CC_Data[-1])

head(CC_Data_scale, n=3)
```
...
```{r}
ind <- sample(2, nrow(CC_Data_scale), replace = TRUE, prob = c(0.7, 0.3))
train.data <- as.data.frame(CC_Data_scale[ind == 1, ])
test.data <- as.data.frame(CC_Data_scale[ind == 2, ])
```
...
```{r}
nn <- neuralnet(formula = Class ~ V1 + V3 + V15 + V18 + V11 + V12 +V5, 
